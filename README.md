...
AcoustiSinc implements four distinct mathematical reconstruction filter topologies in strict 64-bit double precision (`float64` / `complex128` / OpenCL `double2`). All filter kernels, transition bands, and roll-off envelopes scale proportionally relative to the **source sampling rate ($F_s$) and Nyquist limit ($F_{\text{Nyq}} = F_s / 2$)**, delivering identical DSP behavior across **$44.1\text{ kHz}$ Red Book**, **$48.0\text{ kHz}$ Studio/Film**, and high-res ($88.2\text{k} / 96\text{k}$) masters:
